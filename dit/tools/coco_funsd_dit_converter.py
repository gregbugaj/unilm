...
def process(coco_annoations_file:str, output_file:str):
    ...
    # filter out the annotations that are not of specific category 1  - is typically the text category / bbox
    ...
    if filter_annotations:
        ...
        for i in range(len(data['annotations'])):
            if data['annotations'][i]['category_id'] in id_to_keep:
                annotations.append(data['annotations'][i])

        for i in range(len(data['categories'])):
            if data['categories'][i]['id'] in id_to_keep:
    ...
    bounding_box_expansion = False # TODO: Make this a command line argument
    filter_annotation_categories = False # TODO: Make this a command line argument
    ...
    for i in range(len(data['annotations'])):
        ...
        # expand the bounding box by delta pixels, this is to ensure that the bounding box is not too tight(DATA LABELING ISSUE)        
        if bounding_box_expansion:    
            delta = 4
            x = max(0, x)
            w = w + delta
            ann['bbox'] = [x, y, w, h]
